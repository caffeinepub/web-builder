...
export class Backend implements backendInterface {
    ...
    async loadPage(): Promise<string | null> {
        if (this.processError) {
            try {
                const result = await this.actor.loadPage();
    ...
    async savePage(arg0: string): Promise<void> {
        if (this.processError) {
            try {
                const result = await this.actor.savePage(arg0);
                return result;
            } catch (e) {
                this.processError(e);
                throw new Error("unreachable");
